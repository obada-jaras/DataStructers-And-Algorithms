...
//	@Override
		...
		try {
			startRoot = new BorderPane();
				VBox vbox = new VBox(30);
					...
					BTN_maxFrequency = new Button("Max Frequency");
					BTN_maxFrequency.getStyleClass().add("disabled-button");
					
					BTN_babiesInYear = new Button("Babies In A Year");
					BTN_babiesInYear.getStyleClass().add("disabled-button");
					...
					HBox hbox = new HBox(20);
						...
						BTN_exportData = new Button("Export Data");
						BTN_exportData.getStyleClass().add("disabled-button");
						
						Button BTN_exit = new Button("Exit");
						BTN_exit.getStyleClass().add("exit-button");
												
						hbox.getChildren().addAll(BTN_exportData, BTN_exit);
						hbox.setAlignment(Pos.CENTER);
					
					vbox.getChildren().addAll(BTN_readFiles, line1, BTN_searchForName, BTN_maxFrequency, BTN_babiesInYear, line2, hbox);
			...
			BTN_babiesInYear.setOnAction(e -> {
				if (FLAG_fileWasRead)
					babiesInYearWindow(new Stage());
			});
			
			
			BTN_exportData.setOnAction(e -> {
				if (FLAG_fileWasRead) {
					...
					if(selectedDirectory != null){
						Driver.exportData(Driver.getTree(), selectedDirectory.getAbsolutePath() + "\\exported.txt");
						
						
						BorderPane pane = new BorderPane();
						Label LBL = new Label("Data Was Exported..");
						LBL.setStyle("-fx-text-fill: -style1-secondaryColor; -fx-font-weight: bold;");
						pane.setCenter(LBL);
						Scene scene = new Scene(pane, 400, 50);
						scene.getStylesheets().add(getClass().getResource("application.css").toExternalForm());
						Stage stage = new Stage();
						stage.setScene(scene);
						stage.setTitle("Data Was Exported..");
				...
				if (!(listOfFiles.isEmpty())) {
					...
					if (ability == "done") {
						...
						if (message.equals("")) {
							...
							BTN_babiesInYear.getStyleClass().add("enabled-button");					
							BTN_exportData.getStyleClass().add("enabled-button");	
		...
		try {
			...
			BTN_search.setOnAction(e -> {
				if (tf.getText() != "" && (tBTN_male.isSelected() || tBTN_female.isSelected())) {	
					...
					Record record = Driver.find(name, gender);
					...
					if (record != null) {
						...
						LBL_avgFreq.setText("Average Frequencies: " + String.format("%.2f", Driver.getNameAvgFrequencies(name, gender)));
						

						
						LinkedList list = record.getFrequency();
	...
	public void maxFreqWindow(Stage primaryStage) {
		try {			
			BorderPane root = new BorderPane();
				...
				Record record = Driver.getNameWithMaxFrequency(Driver.getTree());
				...
				if (record != null) {
					...
					LBL_name.setText(record.getName().getName() + " (" + genderr + ")\n");
					LBL_avgFreq.setText("Average Frequencies: " + String.format("%.2f", record.getFrequency().getAvgFrequencies()));
	...
	public void babiesInYearWindow(Stage primaryStage) {
		try {			
			BorderPane root = new BorderPane();
				VBox vbox = new VBox(40);
					HBox hbox = new HBox(20);
						TextField tf = new TextField();
						tf.setPromptText("Year");
						Button BTN = new Button("Get Number Of Babies");
					...
					vbox.getChildren().addAll(hbox, LBL);
			...
			BTN.setOnAction(e -> {
				if (tf.getText() != "") {
					...
					int number = Driver.getTotalNumberInYear(year);
					
					if (number > 0)
						LBL.setText("Number Of Babies in " + year + ": " + number);
					else 
						LBL.setText("No Records Found In " + year);	
				}
			});
	...
	private String filesChosen(LinkedList listOfFiles) {
		for (int i = 0; i < listOfFiles.size(); i++) {
			...
			for (int j = i+1; j < listOfFiles.size(); j++) {
				if (year == ((FileYear)listOfFiles.getAt(j)).getYear())
					return "There are more than one file with year " + year;
			}
		}
